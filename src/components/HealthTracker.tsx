...
interface HealthTrackerProps {
  ...
  onSetHealth: (data: { sleep_hours?: number; running_km?: number; running_minutes?: number }) => void;
  isReadOnly?: boolean;
}
...
export function HealthTracker({ healthData, onSetHealth, isReadOnly = false }: HealthTrackerProps) {
  ...
  const getSleepQuality = (hours: number) => {
    if (hours >= 7 && hours <= 9) return { label: 'Optimal', color: 'text-success' };
    if (hours >= 6 && hours < 7) return { label: 'Okay', color: 'text-primary' };
    if (hours < 6) return { label: 'Low', color: 'text-destructive' };
    return { label: 'Too much', color: 'text-warning-foreground' };
  };
